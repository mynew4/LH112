...
namespace MaNGOS
{
    namespace XP
    {
        ...
        inline uint32 Gain(Player *pl, Unit *u)
        {
			...
			float horde_rate = (pl->GetTeam() == HORDE)  ? sWorld.getConfig(CONFIG_FLOAT_RATE_XP_HORDE) : 1.0f;
			return (uint32)(xp_gain*sWorld.getConfig(CONFIG_FLOAT_RATE_XP_KILL)*horde_rate);
